...
# Script pour installer et finaliser Stripe CLI
# Usage: ./scripts/installer-et-finaliser-stripe.sh

echo "🚀 Installation et finalisation de Stripe CLI"
echo "=============================================="
echo ""
...
if command -v stripe &> /dev/null; then
    ...
    echo "📥 Téléchargement de Stripe CLI..."
    ...
    VERSION="1.34.0"
    URL="https://github.com/stripe/stripe-cli/releases/download/v${VERSION}/stripe_${VERSION}_mac-os_${ARCH_TYPE}.tar.gz"
    
    cd /tmp
    rm -f stripe.tar.gz stripe
    ...
    if [ -f stripe.tar.gz ] && [ -s stripe.tar.gz ]; then
        ...
        if [ -f stripe ]; then
            ...
            echo ""
            echo "   🔐 Installation (nécessite votre mot de passe)..."
        ...
        echo "   2. Télécharger : stripe_X.X.X_mac-os_${ARCH_TYPE}.tar.gz"
...
echo "   (Appuyez sur Ctrl+C après avoir copié le secret)"
echo ""
