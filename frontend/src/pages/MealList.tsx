  ...
  return (
    <div style={{ padding: '2rem', maxWidth: '1200px', margin: '0 auto' }}>
        ...
        {meals.map((meal) => (
          <div key={meal.id} style={{ border: '1px solid #ddd', borderRadius: '8px', overflow: 'hidden' }}>
            ...
            <div style={{ padding: '1rem' }}>
              ...
              <Link to={`/meals/${meal.id}`} style={{ display: 'inline-block', marginTop: '0.5rem', padding: '0.5rem 1rem', background: '#007bff', color: 'white', textDecoration: 'none', borderRadius: '4px' }}>
                Voir les détails
              </Link>
            </div>
          </div>
      ...
      {meals.length === 0 && !loading && (
        <p style={{ textAlign: 'center', marginTop: '2rem' }}>Aucun repas disponible</p>
      )}
